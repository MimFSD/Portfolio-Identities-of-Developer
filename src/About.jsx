


const About = () => {
    
    return (
        <div className='max-w-[370px] md:max-w-[540px] lg:max-w-[1540px] mx-auto px-4 md:px-8 py-8 md:py-10 mt-20  mb-10'>
                <h2 className="hover:text-blue-600 text-xl text-center md:text-[40px]  font-bold mb-10"> About Me</h2>
                    <div className='text-center'>
                        <p>
                            Passionate about Probelm solving, software enginnering , web development, and creating innovative web solutions.
                            I solve around 800+ problem solve in varrious online judge.
                            I am also a professional MERN Stack Web Developer with expertise in building and maintaining web applications using MongoDB, Express.js, React.js, and Node.js. My skills and experience allow me to utilize the latest technologies and methodologies in modern web development.
                        </p>
                        <p>
                            I make eye caching site.I like help other which i learn.Cooperative with others team members.And Contribute any project by idea.
                        </p>
                    </div>
                </div>
    );
};

export default About;